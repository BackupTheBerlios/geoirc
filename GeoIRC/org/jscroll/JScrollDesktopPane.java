...
public class JScrollDesktopPane extends JPanel implements DesktopConstants {
     ...
     * @param y y coordinates of internal frame within the scrollable desktop
     */
    public JInternalFrame add(JInternalFrame f, int x, int y) {
        JScrollInternalFrame sif = getWrappedFrame( f );
        desktopMediator.add( sif, x, y );
        return sif;
    }
    
    public void add( JScrollInternalFrame f )
    {
        add( f, -1, -1 );
    }
    
    public void add( JScrollInternalFrame f, int x, int y )
    {
        f.pack();
        f.saveSize();
