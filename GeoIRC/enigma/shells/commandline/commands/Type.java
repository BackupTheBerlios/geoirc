...
public class Type {
    public static void main(String[] args) {
        if (args.length == 0) {
            System.out.println(Util.msg(Type.class, "usage.info"));
            return;
        }
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (expanded == null || expanded.length == 0)
                System.out.println(Util.msg(Type.class, "file.not.found", new String[] { args[i] }));
            else
    ...
    public static void type(File[] paths) {
        if (paths.length == 1)
            ...
            for (int i = 0; i < paths.length; i++) {
                System.out.println(Util.msg(Type.class, "file.name", new File[] { paths[i] }));
                System.out.println();
                type(paths[i]);
                if (i < paths.length - 1)
                    System.out.println();
    ...
    public static void type(File path) {
        try {
            if (!path.exists()) {
                System.out.println(Util.msg(Type.class, "file.not.found", new Object[] { path }));
                return;
            }
            if (path.isDirectory()) {
                System.out.println(Util.msg(Type.class, "file.is.directory", new Object[] { path }));
                return;
            }
