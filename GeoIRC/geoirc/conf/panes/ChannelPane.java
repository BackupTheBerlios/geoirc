...
public class ChannelPane extends BaseSettingsPanel implements Storable
{
    ...
    private AddChannelActionHandler addChannelHandler;
    private SetChannelActionHandler setChannelHandler;
    ...
    public void initialize()
    {
        ...
        serverList.addListSelectionListener(new ServerListSelectionHandler(this));
        ...
        addComponent(addServerButton, 2, 5, 1, 1, 0, 0, GridBagConstraints.SOUTHWEST);
        delServerButton = new JButton("Delete");
        delServerButton.addActionListener(new DelServerActionHandler(this));
        addComponent(delServerButton, 3, 5, 1, 1, 1, 0, GridBagConstraints.SOUTHWEST);

        addComponent(new TitlePane("channels of selected server"), 0, 6, 10, 1, 0, 0);
        ...
        addChannelButton.addActionListener(addChannelHandler);
        addComponent(addChannelButton, 2, 10, 1, 1, 0, 0, GridBagConstraints.SOUTHWEST);
        delChannelButton = new JButton("Delete");
        delChannelButton.addActionListener(new DelChannelActionHandler(this));
        addComponent(delChannelButton, 3, 10, 1, 1, 1, 0, GridBagConstraints.SOUTHWEST);
    ...
    private void fillServerMap()
    {
        ...
        {
            String type = settings_manager.getString(nodePath + "type", (String)typeRule.getValue());
            String hostname = settings_manager.getString(nodePath + "hostname", null);
            if (hostname == null)
                break;

            int port = settings_manager.getInt(nodePath + "port", ((Integer)portRule.getValue()).intValue());
            ...
            {
                String channelName = settings_manager.getString(childNodePath + "name", "");
                boolean autojoin = settings_manager.getBoolean(childNodePath + "autojoin", true);
    ...
    public void handleServerInput()
    {
        ...
        {
            if (hostName.isValid() && hostPort.isValid() && hostName.isEmpty() == false && hostPort.isEmpty() == false)
            {
        ...
        public void valueChanged(ListSelectionEvent e)
        {
            if (e.getValueIsAdjusting() == false)
            {
                if (serverList.getSelectedIndex() > 0)
                {
                    ...
                    applyValuesToListModel(channelListModel, server.getChannels().toArray());
    ...
    class AddServerActionHandler implements ActionListener
    {
        ...
        {
            IRCServer server =
                new IRCServer((String)typeRule.getValue(), hostName.getText(), Integer.parseInt(hostPort.getText()));
    ...
    class AddChannelActionHandler implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent arg0)
        {
            if (serverList.getSelectedIndex() > 0)
            {
                ...
                IRCServer server = (IRCServer)servers.get(name);
                int pos = channelList.getSelectedIndex() + 1;
                Channel channel = new Channel(channelName.getText(), channelJoin.isSelected());
    ...
    /**
     * Listener for the "Set Channel" button
     */
    class SetChannelActionHandler implements ActionListener
    {
        private ChannelPane pane;

        public SetChannelActionHandler(ChannelPane pane)
        {
            this.pane = pane;
        }
        ...
        public void actionPerformed(ActionEvent arg0)
        {
            int pos = channelList.getSelectedIndex();
            String name = (String)serverList.getSelectedValue();
            IRCServer server = (IRCServer)servers.get(name);
            Channel selChannel = (Channel)server.getChannels().get(pos - 1);
            selChannel.setName(channelName.getText());
            selChannel.setAutojoin(channelJoin.isSelected());
            applyValuesToListModel(channelListModel, server.getChannels().toArray());
            channelList.setSelectedIndex(pos);
            channelList.ensureIndexIsVisible(pos);
        }
    ...
    public boolean saveData()
    {
        ...
        while (it.hasNext())
        {
            ...
            settings_manager.setString(nodePath + "hostname", server.getHostname());
            ...
            while (cIt.hasNext())
            {
                ...
                settings_manager.setString(childNodePath + "name", channel.getName());
                settings_manager.setBoolean(childNodePath + "autojoin", channel.isAutojoin());
                a++;
            }
            i++;
        }
    ...
    private void switchAddButton(boolean isAdd, boolean state)
    {
        if (isAdd)
        {
            addChannelButton.removeActionListener(setChannelHandler);
            addChannelButton.setText("Add");
            addChannelButton.addActionListener(addChannelHandler);
        }
        else
        {
            addChannelButton.removeActionListener(addChannelHandler);
            addChannelButton.setText("Set");
            addChannelButton.addActionListener(setChannelHandler);
