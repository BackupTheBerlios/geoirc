...
public class GeneralPane extends BaseSettingsPanel implements Storable
{
     ...
     * @param name
     */
    public GeneralPane(XmlProcessable settings, GeoIRCDefaults valueRules, String name)
    {
    ...
    public void initialize()
    {
        ...
        save_handler.register(
            addComponent(new JValidatingTextField(pattern, value, 180), 1, 1, 1, 1, 1, 0),
            path + "name");
        ...
        save_handler.register(
            addComponent(new JValidatingTextField(pattern, value, 150), 1, 2, 1, 1, 1, 0),
            path + "nick1");

        value = settings_manager.getString(path + "nick2", "");
        addComponent(new JLabel("Nick Name2"), 0, 3, 1, 1, 0, 0);
        save_handler.register(
            addComponent(new JValidatingTextField(pattern, value, 150), 1, 3, 1, 1, 1, 0),
            path + "nick2");

        value = settings_manager.getString(path + "nick3", "");
        addComponent(new JLabel("Nick Name3"), 0, 4, 1, 1, 0, 0);
        save_handler.register(
            addComponent(new JValidatingTextField(pattern, value, 150), 1, 4, 1, 1, 1, 0),
            path + "nick3");
        ...
        save_handler.register(
            addComponent(new JValidatingTextField(rule.getPattern(), value, 150), 1, 6, 1, 1, 1, 0),
            path + "username");

        value = settings_manager.getString(path + "os", "");
        addComponent(new JLabel("OS"), 0, 7, 1, 1, 0, 0);
        save_handler.register(addComponent(new JValidatingTextField(".+", value, 180), 1, 7, 1, 1, 1, 0), path + "os");
